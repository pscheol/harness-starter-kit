...
5. **아키텍처 변형** — 한 스택 안에서도 레이아웃은 하나가 아니다. 아키텍처에 종속되는 파일은 4개뿐이라 `templates/stacks/<stack>/arch/<variant>/`에 그 4개만 두고 선택한 하나를 얹는다. 설치 파일 수는 변형과 무관하다. 실제 수는 고른 에이전트에 달렸다 — core 38 + claude 14 · codex 14 · cursor 9 · kiro 31.
