...
> SDD 3단계 — 작업 생성. design을 실행 가능한 tasks/active/<feature>.md 로 분해한다.
...
핵심:
1. `scripts/new-feature.sh <slug> <feature> --stage=tasks` 로 tasks 파일을 만든다(design 이 없으면 스크립트가 거부한다). 그다음 requirements의 User Story(P1/P2/P3)와 design(컴포넌트·API·데이터)을 작업으로 변환.
