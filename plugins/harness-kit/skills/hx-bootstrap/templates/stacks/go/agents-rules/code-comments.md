...
5. **절차** — 본문을 훑어서는 무슨 일을 하는지 안 잡히는 함수. doc comment 한 줄에 더해, 아래 조건에 걸리면 처리 흐름을 붙인다.
...
반대로 저장·조회만 하는 유스케이스, 접근자·위임·매퍼에는 쓰지 않는다. doc comment 한 줄로 끝나는 함수에 흐름을 억지로 붙이지 않는다.
들여쓴 목록(`//  1.`)은 godoc이 목록으로 렌더링하므로 숫자 앞 공백 두 칸을 유지한다.
...
func (s *Service) Renew(ctx context.Context, id domain.SubscriptionID) (*domain.RenewalResult, error)
```
...
- 분기가 얽힌 함수인데 doc 한 줄뿐이고 흐름이 없는가 → 읽는 사람이 본문을 역추적해야 한다. 추가한다.
