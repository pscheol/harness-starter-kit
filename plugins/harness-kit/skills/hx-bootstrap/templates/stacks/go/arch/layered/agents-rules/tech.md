...
레이아웃·레이어 계약의 원본은 `ARCHITECTURE.md`다. 이 문서는 **무엇으로 만들고 어떻게 돌리는가**만 다룬다.
...
| HTTP | **net/http** (+ `chi` 등 경량 라우터) | `internal/handler`만 이 의존을 갖는다 |
| DB 접근 | **pgx**(PostgreSQL) 또는 `database/sql` + 드라이버 | 쿼리 생성은 `sqlc`(컴파일 타임 검증) 선택. 쿼리는 `internal/repository`에서 끝난다 |
| Migration | golang-migrate 또는 goose | `migrations/`에 SQL 파일. 롤백 스크립트 포함 |
| DB | **관계형 DB 선택**(PostgreSQL/MySQL 등) | 메타데이터·권한의 단일 소스 |
| 로깅 | **log/slog**(표준) | 구조화 로그. `internal/logger`에서 구성하고 주입. 전역 지양 |
| 설정 | env 로드(표준 `os` + 파서, `envconfig`·`viper` 선택) | **`internal/config` 한 곳**에서 구조체 1개로 |
| 포맷 | **gofumpt**(gofmt 상위집합) | 포맷 드리프트는 게이트에서 차단 |
| 린트 | golangci-lint (govet·staticcheck·errcheck·revive·depguard·gosec·bodyclose·contextcheck) | depguard = 레이어 방향 강제 |
| 취약점 | **govulncheck** | 정기·CI 실행 |
| 테스트 | 표준 **testing**(테이블 주도) + `-race` · (선택) `testcontainers-go` | 목보다 손수 짠 fake 우선. 인터페이스는 소비자(service)가 선언 |
| API 문서 | OpenAPI 3.x(`api/` 에 스펙) + `oapi-codegen`(선택) | spec-first 권장(스펙이 계약 원본) |
...
linters:
  enable:
    ...
    - depguard       # 레이어 의존 강제(ARCHITECTURE.md §3.2)
    - misspell
    - unconvert
...
linters-settings:
  ...
  # depguard 레이어 규칙(service·repository·model → net/http 금지,
  # handler → repository 직접 import 금지)은 ARCHITECTURE.md §3.2 참조(원본)
...
go test -race ./internal/service/...             # 한 레이어만
...
- 환경변수는 `.env.example` 참조(`.env`는 git-ignore, 실값 commit 금지). 값 읽기는 **`internal/config` 한 곳**에서만 — `handler`·`service`·`repository`의 `os.Getenv` 직접 호출 금지. `main`이 1회 로드해 주입한다.
