...
5. **절차** — 본문을 훑어서는 무슨 일을 하는지 안 잡히는 함수. **책임 한 줄**을 적고, 아래 조건에 걸리면 처리 흐름까지 적는다.
...
단계별 흐름(`처리 흐름: 1. … 2. …`)은 **복잡한 함수의 것**이다. 아래 **둘 중 하나**면 쓴다.

- **절차 복잡도** — 조건 분기·외부 연동·상태 전이가 얽혀, 본문을 훑어서는 절차가 안 잡힌다.
- **순서 의존** — 단계 순서를 바꾸면 버그가 난다(권한 → 검증 → 저장 → 보상). 이때는 각 단계에 **순서의 이유**를 적는다.

**5단계 이내**로, 각 단계는 "무엇을 — 왜"로 한 줄. 여섯 단계가 필요하면 주석이 아니라 함수를 쪼갠다.
반대로 저장·조회만 하는 유스케이스, getter·위임·매퍼에는 쓰지 않는다. 책임 한 줄로 끝나는 함수에 흐름을 억지로 붙이지 않는다.
...
```kotlin
// ✓ 순서 의존 — 순서가 곧 정합성이라 각 단계에 순서의 이유를 적는다
/**
 * 주문을 확정하고 재고를 차감한다.
 *
 * 처리 흐름:
...
```kotlin
// ✓ 절차 복잡도 — 순서를 바꿔도 되지만 분기가 얽혀 본문만 봐선 안 잡힌다
/**
 * 구독 갱신 결제를 처리한다.
 *
 * 처리 흐름:
 * 1. 크레딧 차감 — 잔액이 결제액을 덮으면 PG를 아예 타지 않는다.
 * 2. 잔액 카드 결제 — 실패는 3회까지 재시도, 그 뒤는 연체 상태로 넘긴다.
 * 3. 구독 기간 연장 — 전액 성공만 인정한다. 부분 결제는 연장하지 않는다.
 * 4. 영수증 발행 — 실패해도 결제는 되돌리지 않고 재발행 큐에 넣는다.
 */
@Transactional
fun renew(subscriptionId: SubscriptionId): RenewalResult { ... }
```
...
- 코드를 한국어로 옮긴 문장인가 → 삭제.
- 분기가 얽힌 함수인데 책임 한 줄도 흐름도 없는가 → 읽는 사람이 본문을 역추적해야 한다. 추가한다.
- 흐름 주석의 각 단계에 "왜"가 있는가, 5단계 이내인가 → 아니면 삭제하거나 함수를 쪼갠다.
