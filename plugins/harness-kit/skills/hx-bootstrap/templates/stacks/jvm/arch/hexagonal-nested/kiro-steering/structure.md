...
요약:
- 멀티모듈 헥사고날(도메인 컨테이너 아래 중첩): `bootstrap`·`common`·`core` + `{{PROJECT_SLUG}}-domain/<ctx>/{domain,application,primary,infra}`.
- 모듈 경로는 `:{{PROJECT_SLUG}}-domain:<ctx>:infra` 형태. 컨텍스트를 최상위로 올리는 형태는 기본 `hexagonal` 변형.
