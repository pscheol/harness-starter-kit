...
요약:
- Gradle 멀티모듈. 킷이 강제하는 것은 등급 간 의존 방향 하나이고, 모듈을 무엇으로 자를지·어떻게 이름 붙일지는 프로젝트가 정한다(도메인·연동 대상·기술 관심사·공개 표면 중 선택. `client-` 같은 접두사 강제 없음).
- 등급은 셋: 실행(1개, `bootJar`) → 구성(N개) → 공유(계약·공용 모델). 의존은 위→아래 단방향. `구성 → 실행`·`공유 → 위`·`구성 A ↔ 구성 B`는 컴파일 실패. 구성 모듈 간 의존이 꼭 필요하면 `structure.md` §3 표에 기록한다.
