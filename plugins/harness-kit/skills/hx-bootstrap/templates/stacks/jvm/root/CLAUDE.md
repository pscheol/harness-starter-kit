...
- 복잡한 작업은 `.agents/docs/<slug>-specs/tasks/active/`에 계획을 남기고, 변경은 `bash scripts/verify.sh`로 검증한다. Stop hook은 `fast`(구조 점검만)로 돌므로, 빌드·테스트를 포함한 `full`은 커밋·푸시 전에 직접 실행해 통과시킨다.
