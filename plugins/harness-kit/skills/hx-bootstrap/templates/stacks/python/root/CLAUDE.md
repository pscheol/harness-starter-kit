...
Python 백엔드(ASGI, FastAPI) · src 레이아웃 헥사고날. 검증 게이트 `bash scripts/verify.sh`(ruff → mypy → lint-imports → pytest). 명령·구조·버전 상세는 `ARCHITECTURE.md`·`.agents/rules/tech.md` 원본으로 위임.
