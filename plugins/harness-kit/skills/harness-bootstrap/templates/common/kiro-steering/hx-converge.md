---
inclusion: manual
description: 'SDD 회수 — 구현 후 발견된 잔여·후속 작업을 tasks에 append-only(## Phase N: Convergence)로 되살린다.'
---

<!-- HARNESS STARTER KIT · 얇은 트리거. 정본: .agents/rules/sdd-workflow.md (/hx-converge 절) -->

SDD 워크플로 **정본** `.agents/rules/sdd-workflow.md` 의 **`/hx-converge`** 절을 로드해 그대로 수행한다.
...
4. **스펙 결함이면 회수가 아니라 되돌림** — 요구 누락은 `/hx-specify`, 모호는 `/hx-clarify`, 설계 변경은 `/hx-plan`. `/hx-converge`는 **작업 목록 복구**지 스펙 수정이 아니다.
