...
<!-- HARNESS STARTER KIT · 얇은 트리거. 정본: .agents/rules/sdd-workflow.md (/hx-checklist 절) -->

SDD 워크플로 **정본** `.agents/rules/sdd-workflow.md` 의 **`/hx-checklist`** 절을 로드해 그대로 수행한다.
...
> 이 명령은 **한 문서의 품질**을 본다. requirements↔design↔tasks **교차 정합성**은 `/hx-analyze`.

대상(제품·기능 + 도메인, 생략 시 최근 requirements·requirements-quality): $ARGUMENTS
