---
description: 하네스 컨텍스트를 로드한다 (작업 시작 시 실행)
---
...
1. `AGENTS.md` — 진입점 / 목차
2. `.agents/rules/*` — 규칙 정본 (guardrails·security·api-standards 등, 3 에이전트 공유 공통)
3. `.agents/docs/README.md` — SDD 기록 시스템(SSOT) 개요
4. `ARCHITECTURE.md` — 모듈·레이어 맵
5. `.agents/docs/decisions/core-beliefs.md` — 핵심 신념
...
- 변경은 `scripts/verify.sh`(스택 게이트 정본)로 검증한다.

그 다음 사용자의 실제 요청을 수행하라: $ARGUMENTS
