...
if ! ls -d "$DOCS"/product-*-specs >/dev/null 2>&1; then
  echo "ℹ 제품 SDD 폴더(.agents/docs/product-*-specs) 없음 — 신선도 점검 대상 없음(OK)"; exit 0
fi
...
for f in "$DOCS"/product-*-specs/requirements/*.md "$DOCS"/product-*-specs/design/*.md; do
  [ -e "$f" ] || continue
  is_meta "$f" && continue
  case "$(status_of "$f")" in
...
done < <(grep -rnE '\[NEEDS CLARIFICATION:' "$DOCS"/product-*-specs 2>/dev/null \
           | grep -v '/_template.md:' \
...
for f in "$DOCS"/product-*-specs/tasks/active/*.md; do
  [ -e "$f" ] || continue
  is_meta "$f" && continue
