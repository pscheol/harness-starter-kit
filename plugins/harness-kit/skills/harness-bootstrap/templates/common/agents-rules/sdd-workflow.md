...
이 파일은 **{{PROJECT_NAME}} SDD 워크플로의 단일 정본**이다. Claude Code 슬래시 명령(`.claude/commands/*`),
Codex, Kiro(`.kiro/steering/sdd-workflow.md`)는 모두 이 정본을 참조하는 **얇은 트리거**다("1곳 + N트리거").

원리: **스펙이 정본, 코드는 그 산출물**이다(Spec-Driven Development의 핵심). 기능은 requirements → design → tasks 순으로,
**제품(바운디드 컨텍스트) 폴더 안에서** 진행한다. 산출 위치:
...
2. **Constitution Check (게이트)**: 설계가 규칙 정본을 위반하지 않는지 점검. 위반이 불가피하면 design의 "Complexity/대안" 섹션에 **정당화**를 남긴다(정당화 없으면 진행 금지).
...
한 문서의 품질은 `/hx-checklist`가, **세 문서(requirements↔design↔tasks) 사이의 정합성**은 `/hx-analyze`가 본다.
...
2. **탐지 분류 ↔ 심각도**:
   ...
   | 커버리지 갭(역방향) | 스펙 근거 없는 작업·설계(요구 없는 산출물) | HIGH |
...
명령(슬래시)이 없는 에이전트(Codex 등)도 이 정본의 절차·산출 위치·게이트를 그대로 따른다.
