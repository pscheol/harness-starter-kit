...
# 실행 계획 (Exec Plans) = SDD: tasks 단계

`tasks/`(제품 폴더 안)는 SDD의 **tasks 단계**다(requirements → design → **tasks**).
기능별 작업은 `active/<feature>.md`에 둔다. 템플릿: `_template.md`. 스펙 흐름: `../index.md`(이 제품 색인).

```text
tasks/                 (product-<slug>-specs/ 안)
...
- **잔여 작업 회수(append-only)**: 구현 후 남거나 새로 드러난 작업은 기존 Phase를 고쳐 쓰지 않고 파일 끝에 **`## Phase N: Convergence`** 를 덧붙여 회수한다(`/hx-converge` · 정본 `sdd-workflow.md`). 근거는 `scripts/check-spec-freshness.sh` 리포트.

## 생애주기 & 완료 게이트 (사용자 검증 필수)

상태/위치 전이: `active/` → `check/` → `completed/` (상태값도 동일하게 `active`/`check`/`completed`).

1. **active**: 작업 진행 중. 파일은 `active/`.
2. **check** (= 사용자 검증 대기 단계): DoD와 `scripts/verify.sh`를 충족하면 에이전트는
...
> 검증(verify.sh) 통과는 완료의 **필요조건**일 뿐, 충분조건이 아니다(최종 승인은 사람).
>
