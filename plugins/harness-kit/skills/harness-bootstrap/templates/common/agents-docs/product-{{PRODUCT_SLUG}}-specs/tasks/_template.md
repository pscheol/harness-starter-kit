...
- **`[P]`**: 병렬 가능 표시 — **다른 파일**이고 **선행 의존이 없을 때만**. 같은 파일을 건드리면 `[P]` 금지(순차).
...
- **User Story 간**: 각 스토리는 Foundational 이후 시작하며 서로 **독립 테스트 가능**해야 한다(교차 의존이 독립성을 깨면 안 됨).
...
1. DoD와 `scripts/verify.sh`를 충족하면 상태를 **`check`**로 바꾸고, 파일을 **`check/`로 이동**한다(active → check).
