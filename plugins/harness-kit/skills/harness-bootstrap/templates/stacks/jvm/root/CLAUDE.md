...
Kotlin/Java + Spring Boot(JVM). 아키텍처 변형은 설치 시 선택하며 원본은 `ARCHITECTURE.md`다. 검증 게이트 `bash scripts/verify.sh`(= exec-plan 점검 + `./gradlew check`). 명령·구조·버전 상세는 `ARCHITECTURE.md`·`.agents/rules/tech.md` 원본으로 위임.
...
- 복잡한 작업은 `.agents/docs/product-<slug>-specs/tasks/active/`에 계획을 남기고, 변경은 `bash scripts/verify.sh`로 검증한다. Stop hook은 `fast`(구조 점검만)로 돌므로, 빌드·테스트를 포함한 `full`은 커밋·푸시 전에 직접 실행해 통과시킨다.
