...
- 서킷브레이커는 **인바운드 API가 아니라 아웃바운드(내가 거는) 의존성 호출**에 적용한다. 인바운드 보호(rate limit 등)는 앞단 리버스 프록시/게이트웨이(선택) 또는 앱 레벨에서 다룬다.
- 적용 대상: 서비스 간 gRPC, 외부 HTTP/오브젝트 스토리지, 외부 Provider 등.
- **구현은 서킷브레이커 라이브러리(예: Resilience4j)로 한다.** 프로토콜 무관 데코레이터이므로 REST·gRPC 모두 적용 가능하며, **외부 호출을 감싸는 아웃바운드 어댑터 메서드에 `@CircuitBreaker`**(+`@TimeLimiter`·제한 `@Retry`·필요 시 `@Bulkhead`)를 붙인다(gRPC `ClientInterceptor` 방식보다 어댑터 경계와 정합). 어댑터의 실제 위치는 아키텍처 변형마다 다르다 — 정본은 `ARCHITECTURE.md`·[`structure.md`](./structure.md).
  ```kotlin
  ...
  ```
- **unary RPC**(요청-응답 1건)에 서킷브레이커를 적용한다. **서버 스트리밍**(예: 스트리밍 응답)은 실패 기준이 모호하므로 CB는 "스트림 개시" 단계에만 한정하거나 생략하고 **deadline + 개시 타임아웃**으로 다룬다.
...
- 상태 전이는 정의된 enum만 따른다(도메인 데이터 정본). 잘못된 전이 차단.
...
- **가상 스레드**(`spring.threads.virtual.enabled=true`, JDK 21+): blocking I/O 동시성 효율을 높이되, 실제 DB 동시성 상한은 **커넥션 풀**이 결정하므로 풀·DB 사이징과 함께 조정한다.
