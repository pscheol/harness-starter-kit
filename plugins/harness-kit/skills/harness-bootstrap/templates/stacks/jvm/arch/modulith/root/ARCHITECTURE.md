...
본 프로젝트는 모듈러 모놀리스다: 하나의 Gradle 모듈·하나의 배포 단위 안에서 패키지가 모듈 경계가 되고,
...
> 기계적 강제 우선. 이 변형에서 아키텍처는 문서가 아니라 모듈 검증 테스트다.
...
- 배포 단위는 하나다. 모듈은 배포 경계가 아니라 소유·변경 경계다.
...
그 패키지의 최상위 타입만 공개 API이고, 하위 패키지는 모듈 내부로 간주되어 다른 모듈이 참조하면 검증이 실패한다.
...
class ModuleStructureTest {
    ...
    @Test
    fun `모듈 문서를 생성한다`() {
        Documenter(modules).writeDocumentation()   // 모듈 다이어그램·의존 목록(빌드 결과물)
    }
}
```
...
- 전달 보장이 필요하면 이벤트 발행 레지스트리(`spring-modulith-events-*`)를 켠다. 미완료 이벤트가 저장되어 재발행되므로 "커밋됐는데 리스너가 죽어 유실"을 막는다. 재발행은 최소 1회 전달이므로 리스너는 멱등해야 한다.
