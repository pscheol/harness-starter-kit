...
Go 백엔드 · 표준 Go 레이아웃(cmd/internal/pkg). **`internal/` 내부 구조는 `ARCHITECTURE.md` 정본**(선택한 변형에 따라 다르다). 검증 게이트 `bash scripts/verify.sh`(fmt → build → vet → golangci-lint → test -race). 명령·구조·버전 상세는 `ARCHITECTURE.md`·`.agents/rules/tech.md` 정본으로 위임.
...
- 레이어 의존은 `internal/` 가시성·import 사이클(컴파일러) + **depguard**(린터)가 강제한다. **인터페이스는 소비자(app)가 선언**한다.
