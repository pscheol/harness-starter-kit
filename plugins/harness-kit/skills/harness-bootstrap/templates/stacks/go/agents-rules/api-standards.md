...
- **성공**: `internal/common`의 `httpx.OK(w, data, requestID)` / 목록 `httpx.Page(w, items, page, requestID)`.
  ...
  ```
- **오류**: 유스케이스는 도메인 에러를 반환하고, **경계(핸들러/미들웨어)가 envelope + status로 변환**한다.
  ```json
...
- `internal/common`의 **`ErrorCode`(타입 있는 상수)가 (머신코드 · HTTP status · 기본 메시지 키)의 단일 매핑 소스**다. 매핑을 여러 곳에 흩지 않는다.
...
Go는 코드에서 스펙이 자동 생성되지 않으므로 **스펙을 계약 정본으로 관리**한다.

- **spec-first 권장**: `api/openapi.yaml`을 정본으로 두고 `oapi-codegen`으로 서버 인터페이스·DTO를 생성한다. 생성 코드는 커밋하고 손편집하지 않는다(생성기 설정을 고친다).
