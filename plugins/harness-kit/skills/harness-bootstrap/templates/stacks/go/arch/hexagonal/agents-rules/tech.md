...
linters-settings:
  ...
  # depguard 레이어 규칙은 ARCHITECTURE.md §4.2 참조(원본)
...
- 환경변수는 `.env.example` 참조(`.env`는 git-ignore, 실값 commit 금지). 값 읽기는 `internal/platform/config` 한 곳에서만 — 하위 패키지의 `os.Getenv` 직접 호출 금지.
