...
> **규칙을 더 걸고 싶어진다면 그건 `feature`로 갈 때가 됐다는 뜻**이다 — 디렉터리 경계가 있어야 규칙이 의미를 갖는다.
...
> 상한은 프로젝트가 정한다. 중요한 것은 **승격 시점이 자동으로 눈에 띄게 만드는 것**이다.
...
- 시그니처를 옮긴 번역투 금지. `//nolint:` 에는 반드시 이유를 적는다.
