...
레이아웃·승격 기준의 정본은 `ARCHITECTURE.md`다. 이 문서는 **무엇으로 만들고 어떻게 돌리는가**만 다룬다.
...
linters-settings:
  ...
  # depguard 최소 규칙(store.go·model.go → net/http 금지)은 ARCHITECTURE.md §3.2 참조(정본).
...
- 환경변수는 `.env.example` 참조(`.env`는 git-ignore, 실값 commit 금지). 값 읽기는 **`internal/app/config.go` 한 곳**에서만 — 다른 파일의 `os.Getenv` 직접 호출 금지. `main`이 1회 로드해 주입한다.
