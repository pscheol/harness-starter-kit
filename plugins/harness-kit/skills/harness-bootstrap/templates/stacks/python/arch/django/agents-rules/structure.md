...
이 프로젝트는 **Django 앱 단위 구조 + 서비스/셀렉터 레이어**를 쓴다.
...
- 계약 선언과 추가 절차는 `ARCHITECTURE.md` §3.2. **새 앱은 `containers`와 `independence.modules` 양쪽에 등록**해야 강제 대상이 된다.
...
`import-linter`가 **레이어 방향과 앱 독립성**을 막는다. 그러나 계약이 **못 잡는** 위반이 있다:
