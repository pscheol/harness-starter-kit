<!-- HARNESS STARTER KIT · {{PROJECT_NAME}} · Python 백엔드(ASGI) · 아키텍처: layered · 플레이스홀더({{PROJECT_NAME}}·{{PROJECT_SLUG}}·{{PACKAGE_NS}}·{{DOMAIN_EXAMPLE}}) 치환 후 사용 -->
...
| 웹 프레임워크 | FastAPI(ASGI) | 대안: Litestar. `api` 레이어만 이 의존을 갖는다 |
| ASGI 서버 | **Uvicorn**(프로덕션은 Gunicorn `UvicornWorker` 등으로 관리) | 워커 수는 커넥션 풀과 함께 결정 |
| DB 접근 | SQLAlchemy 2.0 (async) | 쿼리는 `repositories`에서 끝난다. `services`는 SQL을 모른다 |
| Migration | **Alembic** | 자동 생성 diff는 반드시 사람이 검토 |
| DB | **관계형 DB 선택**(PostgreSQL/MySQL 등) | 메타데이터·권한의 단일 소스 |
| 검증/직렬화 | Pydantic v2 | `schemas`(경계 DTO)·설정 전용. `repositories`·`models`로 내려보내지 않는다 |
...
| 아키텍처 강제 | **import-linter**(`lint-imports`) | `layers`(api→services→repositories→models) + forbidden 계약 |
...
# import-linter 계약(layers·forbidden)은 ARCHITECTURE.md §3.2 참조 — 레이어 패키지를 늘리면 계약에도 등록
```
...
uv run uvicorn {{PACKAGE_NS}}.main:app --reload  # 로컬 실행(8000) — app 객체는 main.py 가 조립
...
lint-imports                 # 레이어 계약(api→services→repositories→models)
pytest                       # 테스트 + 커버리지 임계
```
...
- 환경변수는 `.env.example` 참조(`.env`는 git-ignore, 실값 commit 금지). 값 읽기는 `{{PACKAGE_NS}}/settings.py`(pydantic-settings) 한 곳에서만 한다 — `services`·`repositories`·`models`의 `os.environ` 직접 접근 금지. 설정 객체는 `main.py`에서 1회 생성해 주입한다.
