...
- 환경변수는 `.env.example` 참조(`.env`는 git-ignore, 실값 commit 금지). 값 읽기는 **`{{PACKAGE_NS}}/settings.py`(pydantic-settings) 한 곳**에서만 한다 — `services`·`repositories`·`models`의 `os.environ` 직접 접근 금지. 설정 객체는 `main.py`에서 1회 생성해 주입한다.
