...
이 하네스는 **Django 백엔드 전용**이다. 아래 스택·버전은 **예시일 뿐이며, 최신 안정 버전을 프로젝트에서 확정**한다.
의존성·도구 설정은 **단일 소스 `pyproject.toml`** 에 모으고, 잠금 파일(`uv.lock`/`poetry.lock`)을 **반드시 커밋**한다.

앱 레이아웃·`services`/`selectors` 분리 계약의 정본은 `ARCHITECTURE.md`다. 이 문서는 **무엇으로 만들고 어떻게 돌리는가**만 다룬다.
...
| Migration | **Django migrations** | **코드와 같은 커밋**에 넣는다. 드리프트는 게이트가 차단(`makemigrations --check`) |
...
- **서드파티 Django 앱을 추가하면 `INSTALLED_APPS`·설정·마이그레이션 영향까지 함께 검토**한다(앱 하나가 미들웨어·시그널·모델을 동시에 들여온다).
