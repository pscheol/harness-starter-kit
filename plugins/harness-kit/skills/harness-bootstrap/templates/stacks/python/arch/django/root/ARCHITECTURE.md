...
본 프로젝트는 Django 앱 단위 구조 + 서비스/셀렉터 레이어를 쓴다.
...
> 새 앱을 만들면 (1)의 `containers`와 (2)의 `modules`에 반드시 등록한다. 등록하지 않은 앱은 강제 대상 밖이다(등록 누락 = 강제 누락).
...
- 마이그레이션은 코드와 함께 커밋한다. CI에서 `makemigrations --check --dry-run`으로 드리프트(모델 변경 후 마이그레이션 누락) 를 차단한다.
...
- 비동기 작업(`tasks.py`)은 얇은 래퍼다: 인자 직렬화 + `services` 호출만. 작업은 멱등해야 하고 재시도 정책을 명시한다.
...
- 앱 코드가 `os.environ`을 직접 읽지 않는다. 설정은 `django.conf.settings` 한 곳을 통한다.
...
|---|---|---|
| → `modular` (프레임워크는 유지, 경계만 더 뚜렷하게) | 이미 앱 = 모듈이다. `views/services/selectors/models`를 앱 안에 유지한 채 공개 API(`api.py`)를 명시하고 앱 간 참조를 그쪽으로 몰면 된다. | `independence` 계약은 그대로. `layers` 의 `containers` 에 새 앱을 계속 등록 |
...
- FK가 앱 경계를 넘는 곳이 분리의 실제 비용이다. 전환을 염두에 둔다면 경계를 넘는 FK를 미리 ID 참조로 바꿔둔다.
