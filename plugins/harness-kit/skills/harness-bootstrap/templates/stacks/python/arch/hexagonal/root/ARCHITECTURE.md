...
본 프로젝트는 클린 아키텍처(헥사고날) + DDD + TDD + SOLID를 Python **src 레이아웃** 위에서 구현하고,
...
한 바운디드 컨텍스트(`<ctx>`)는 `primary` · `application` · `domain` · `infra` 네 형제 패키지로 구성되고, 그 아래 `common`·`core` 공유 토대가 깔린다.
...
- **DB 접근**: 표준 CRUD(애그리거트 영속)는 SQLAlchemy 2.0 스타일(`select()`·`session.execute`). 복잡 조회·통계는 별도 read 어댑터로 분리한다. ORM 모델(`infra/persistence/model.py`)과 도메인 애그리거트는 다른 타입이며 `mapper.py`가 변환한다.
...
- CRUD·프로퍼티·위임·매퍼·설정 모듈에는 달지 않는다. 규칙 문서 참조 주석도 쓰지 않는다. yaml·SQL은 값의 근거만 한 줄.
