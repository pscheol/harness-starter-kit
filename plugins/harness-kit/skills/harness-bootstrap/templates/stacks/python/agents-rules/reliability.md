...
- 모든 외부 호출에 timeout 필수. 무한 대기 금지.
  ...
  - `httpx` 기본 timeout은 5초지만 `timeout=None`을 넘기면 무한 대기가 된다 — 명시적으로 금지한다.
...
- **구현은 서킷브레이커 라이브러리**(`purgatory`·`pybreaker`·`aiobreaker` 등)로 하고, 아웃바운드 어댑터(외부 호출 구현) 메서드에 적용한다. 규칙·오케스트레이션 계층은 서킷브레이커를 모른다.
  ```python
...
- FastAPI `BackgroundTasks`는 워커 프로세스와 생명주기를 공유한다 — 재배포·크래시 시 유실된다. 유실되면 안 되는 작업은 반드시 영속 큐를 쓴다.
